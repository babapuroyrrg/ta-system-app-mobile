...
// 85+ oyuncu push bildirimi — uygulama kapalıyken de çalışır
const notifiedServerIds = new Set<string>();
const PLACE_ID = "3231515867";
const PLAYER_THRESHOLD = 85;

async function checkAndPushServerAlerts(): Promise<void> {
  try {
    // Roblox server listesini çek
    const robloxRes = await fetch(
      `https://games.roblox.com/v1/games/${PLACE_ID}/servers/Public?sortOrder=Desc&limit=100`
    );
    if (!robloxRes.ok) return;
    const data = await robloxRes.json() as { data?: Array<{ id: string; playing: number; maxPlayers: number }> };
    const servers = data.data ?? [];

    // Kayıtlı push token'larını al
    const tokenRes = await fetch("http://localhost:3000/api/push/tokens");
    if (!tokenRes.ok) return;
    const { tokens } = await tokenRes.json() as { tokens: string[] };
    if (tokens.length === 0) return;

    for (const server of servers) {
      const isOver85 = server.playing >= PLAYER_THRESHOLD;
      const alreadyNotified = notifiedServerIds.has(server.id);

      if (isOver85 && !alreadyNotified) {
        notifiedServerIds.add(server.id);
        ...
        const messages = tokens.map((token) => ({
          ...
          body: `Bir server şu an ${server.playing}+ kişi oynuyor, hemen sen de katıl!`,
          data: { placeId: PLACE_ID, serverId: server.id },
        }));

        // Expo push endpoint'e toplu gönder (max 100 per batch)
        ...
        console.log(`[Heartbeat] 85+ push gönderildi: server ${server.id}, ${server.playing} oyuncu`);
      } else if (!isOver85 && alreadyNotified) {
        notifiedServerIds.delete(server.id); // Tekrar bildirim gelebilsin
      }
    }
  } catch (e) {
    console.warn("[Heartbeat] Push kontrol hatası:", e);
  }
}

// 60 saniyede bir kontrol et
setInterval(checkAndPushServerAlerts, 60_000);
